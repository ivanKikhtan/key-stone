...
namespace AspNetCoreDemos.DemoShell {
    ...
    public class DemoMenuMeta {
        ...
        public DemoMenuMeta(IWebHostEnvironment env, IHttpContextAccessor httpAccessor, IActionDescriptorCollectionProvider actionDescriptorsProvider) {
            _contentRootPath = env.ContentRootPath;
            _httpAccessor = httpAccessor;

            _rootGroups = JsonSerializer.Deserialize<IList<DemoGroup>>(File.ReadAllText(Path.Combine(_contentRootPath, "menuMeta.json")));
        ...
        string ReadDemoDescriptionFile(Demo demo) {
            ...
            if(File.Exists(path))
                return ReplaceDevExtremeDocLinks(File.ReadAllText(path));
        ...
        private string ReplaceDevExtremeDocLinks(string content) {
            return _htmlLinks.Replace(content, @"](https://js.devexpress.com/DevExtreme$1)");
        }
    }
}
